...
direc = dir([FolderPath,filesep,suffix]); filenames={};
[filenames{1:length(direc),1}] = deal(direc.name);
amount = length(filenames);
...
if nr_of_cores > 1
    ...
    parfor i=1:size(slicedfilename1,2)  % index must increment by 1
        ...
        [x{i}, y{i}, u{i}, v{i}, typevector{i},correlation_map{i}] = ...
            piv_analysis(FolderPath, slicedfilename1{i}, slicedfilename2{i},p,s,nr_of_cores,false);
        toc
    end
...
f = @() piv_analysis(FolderPath, slicedfilename1{i}, slicedfilename2{i},p,s,nr_of_cores,false);
t = timeit(f, 6)
...
r{5,1}= 'Stdev threshold';                                  r{5,2}=15;                  % Threshold for the stdev test
r{6,1}= 'Local median check?';                              r{6,2}=1;                   % 1 = enable local median test
r{7,1}= 'Local median threshold';                           r{7,2}=2.5;                 % Threshold for the local median test
