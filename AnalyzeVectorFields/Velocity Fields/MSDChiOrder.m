...
% pos1 = find(RelDist>OvThresh*CSize);
% pos2 = find(RelDist<=OvThresh*CSize);
% Q = RelDist;
% Q(pos1) = 0;
% Q(pos2) = 1;
% NumObj = [];
% for i = 1:size(Q,1)
%     NumObj(i) = size(Q,2)-sum(isnan(Q(i,:)));
% end
% % Normalize Q
% Q = nansum(Q,2)./NumObj';
...
% Get four point susceptibility:
% Old probably wrong
% % Get Trajectory:
% Trajectory = [];
% Trajectory(:,1) = Q;
% Trajectory(:,2) = [0:size(TrackMat,4)-1]*dt;
% var1 = RelativeDistance(Trajectory(1:end-1,:));
% var2 = Kehl(Trajectory(1:end-1,:));
% Chi = (size(TrackMat,1)*size(TrackMat,2))*(var2-var1.^2);
% Test auf Konsistenz:
