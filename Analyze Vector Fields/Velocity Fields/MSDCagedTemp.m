function [MSDCagedTemp] = MSDCagedTemp(VelField,WSize,dt,im_size,POIx,POIy,SubPxResolution,Sampling,CenterSpeed)


MSDCagedTemp = NaN(WSize,size(VelField,4),10);
Trajectory = [];
...
for k = 1+(WSize-1)/2:Sampling:size(VelField,4)-1-((WSize-1)/2)
    ...
    % Allocate variables:
    MSDCaged = NaN(WSize,size(TrackMat,1)*size(TrackMat,2),10);    
    countNeighbors = 0;
    ...
    MSDCagedTemp(:,idx,:) = nanmean(MSDCaged,2);
end
