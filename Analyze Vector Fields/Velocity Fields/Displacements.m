function [MSD,tau,D2,dt2] = Displacements(Trajectory,Exponent)

% Measures the mean squared displacement (MSD) from a trajectory.
% Was written by Maxime Deforet, May 21 2013. MSKCC.
...
D = zeros(T, T);
D( I + T*(j-1) ) = (sum(abs( Trajectory(I,1:end-1) - Trajectory(j,1:end-1) ).^Exponent, 2)); % Squared distance computation in one line !
D2 = D;
