...
%%%%%%%%%%%%%%%%%%%%%%%%%%% displays %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% figure; imshow(im2,[]); colormap gray;% caxis([0.0 1]) ;%caxis([0.1 .5]) ; 
% title(['truth image',num2str(nx),'*',num2str(ny)]);
%figure; imshow(imconv,[]); colormap gray; title('convHull');hold on        

%%%%%%%%%%% comparisons the resutls
figure('units','normalized','outerposition',[0 0 1 1]);   %%% orientations
a1=w1(:,:,2); a2=w1(:,:,1);  
imshow(im2,[]); colormap gray;%caxis([0.0 1]) ;%caxis([0.1 .5]);  
hold on;
   P1=kron([1;1],Centrex)+scl*kron([1;-1],a1(Cent));
   P2=kron([1;1],Centrey)+scl*kron([1;-1],a2(Cent));
   plot(P1,P2,'g','linewidth',2); hold off %title('tensor orientation');hold off

% figure; %%%% anisotropy
% aniso=sqrt(nu1./nu2); aniso(imconv<0.5)=1; 
% imshow(aniso,[]); 
% colorbar('location','eastoutside'); title('anisotropy')
% colormap jet; caxis([0.0 1]) ;%caxis([0.58 1.02])   
...
% 
% %%%%%%%%%%%%%%%%%%%%%%%%%%% displays %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% figure; imshow(im,[]); colormap gray; caxis([0.0 1]) ;%caxis([0.1 .5]) ; 
% title(['truth image',num2str(nx),'*',num2str(ny)]);
% figure; imshow(imconv,[]); colormap gray; title('convHull');hold on        
% 
% %%%%%%%%%%% comparisons the resutls
% figure;   %%% orientations
% a1=w1(:,:,2); a2=w1(:,:,1);  
% imshow(im,[]); colormap gray;caxis([0.0 1]) ;%caxis([0.1 .5]);  
% hold on;
%    P1=kron([1;1],Centrex)+scl*kron([1;-1],a1(Cent));
%    P2=kron([1;1],Centrey)+scl*kron([1;-1],a2(Cent));
