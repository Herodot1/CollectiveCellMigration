% To do: implement this function into a standard routine for my structures
% Check if orientation /xy-axis is correctly aligned for both types of
% analysis.
% Eventually take averages of the velocity field
...
%     Normalizer = sqrt(VelField(1:CG:end,1:CG:end,1,i).^2 +VelField(1:CG:end,1:CG:end,2,i).^2);
%     figure;
%     imshow(im)
%     hold on
%     quiver(xVelField(1:CG:end,1:CG:end),yVelField(1:CG:end,1:CG:end),squeeze(VelField(1:CG:end,1:CG:end,1,i))./Normalizer,squeeze(VelField(1:CG:end,1:CG:end,2,i))./Normalizer,1)
%     %hold on
%     quiver(xVelField(1:CG:end,1:CG:end),yVelField(1:CG:end,1:CG:end),CellOrientationInterp(:,:,1),CellOrientationInterp(:,:,2),1,'r')
...
% figure;
% polarhistogram(2*pi*(Angle(:))/360,10,'Normalization', 'probability')
