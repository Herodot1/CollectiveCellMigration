...
% Correlation values at 20µm distance - specified time window:
Dist = 20; % autorcorralation function evaluated at specified distance in µm
...
xlabel('Time in min','Fontsize',text_font)
ylabel(sprintf('C(\\Deltar = %d µm)',Dist),'Fontsize',text_font)
%ylim([0 92]);
%ylim([5 23]);
%xlim([0 1236]); % xlim([0 max(VelFieldData(groups(1)).tau)])
xlim(TimeWin*dt)
...
% Reduce analysis to vectors with average speed of more than maybe 2 µm/h ?
...
%             %c.Label.String = 'Speed in µm/h';
%             %set(c,'YLim',[0,1])
%             ylabel(c,'Speed in µm/h','FontSize',24);
%             caxis([0,sp])
...
MinDomainSize = 20; % entspricht rund 2000µm²
% For statistics:
MArea = Area;
...
% Get mean domain size:
% teste 20,10,1 -> am besten 10   <- entspricht rund 2000/1000/100µm²
StdArea = squeeze(nanstd(MArea,[],2));
MArea = squeeze(nanmean(MArea,2));


% Pixelsize in µm/"pixel (entry)" in the tracking matrix
...
ylabel('Domain Size in µm²','FontSize',text_font)
%ylim([0 123]);
%xlim([0 t_max]); % xlim([0 max(VelFieldData(groups(1)).tau)])
%xlim([0 1300])
set(gca, 'Fontsize',text_font)
